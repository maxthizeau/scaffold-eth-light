...
const config: HardhatUserConfig = {
  ...
  typechain: {
    outDir: '../react-app/src/generated/contract-types',
  },
};
...
task('copyTypes', 'Copy the types from front to hardhat so we can use them in tests', async (_, { ethers }) => {
  const source = path.join('../', 'react-app/src/generated/contract-types');
  const target = path.join('./', 'helpers/types');
