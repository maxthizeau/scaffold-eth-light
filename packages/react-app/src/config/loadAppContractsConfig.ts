 ...
 */
const externalContractsPromise = import(
  '../generated/contracts/external_contracts'
)
 ...
 * @returns
 */
export const loadAppContractsConfig =
  async (): Promise<TContractLoaderConfig> => {
    ...
    try {
      config.externalContracts = ((await externalContractsPromise).default ??
        {}) as unknown as TExternalContracts
    } catch {
      config.externalContracts = undefined
      console.error(
        '😶 No external contracts file found: /generated/contracts/external_contracts'
      )
    }
    return config
  }
